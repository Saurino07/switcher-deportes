...
async function recoverLocalCamera(reason=""){if(recovering||document.hidden)return;const track=localStream?.getVideoTracks?.()[0];if(track&&track.readyState==="live"&&!track.muted){try{await local.play();return}catch{}}recovering=true;status("V17.8 · recuperando CAM1",reason);try{localStream?.getTracks().forEach(t=>t.stop());localStream=await openLocalCamera();for(const id of [...cameraPeers.keys()])closeCameraPeer(id);if(program==="cam1")setRenderSource(local);watchDirector();if(replayEnabled&&program==="cam1")startBuffer(localStream);await db?.ref(`switcher/${game}/cameras/cam1`).update({online:true,recoveredAt:firebase.database.ServerValue.TIMESTAMP});status("V17.8 · CAM1 RECUPERADA")}catch(e){status(`CAM1 no disponible: ${e.message}`)}finally{recovering=false}}
...
async function promote(s,token){if(token!==switchToken||pendingRemote!==s)return;const old=activeRemote;activeRemote=s;pendingRemote=null;setRenderSource(s.video);program=s.cam;setProgramAudio(s.stream);status(`V17.8 · ${s.cam.toUpperCase()} AL AIRE`);if(replayEnabled)startBuffer(s.stream);setTimeout(()=>destroyRemote(old),250)}
...
async function switchProgram(cam){requestedCam=cam||"cam1";const token=++switchToken;if(pendingRemote){await destroyRemote(pendingRemote);pendingRemote=null}if(cam==="cam1"){const old=activeRemote;activeRemote=null;program="cam1";setRenderSource(local);setProgramAudio(null);status("V17.8 · CAM1 LOCAL AL AIRE");if(replayEnabled)startBuffer(localStream);setTimeout(()=>destroyRemote(old),250);return}if(activeRemote?.cam===cam)return;const target=activeRemote?.video===front?back:front;status(`V17.8 · preparando ${cam.toUpperCase()}`);try{pendingRemote=await buildRemote(cam,target,token)}catch(e){status(`No conecta ${cam}: ${e.message}`);clearTimeout(retryTimer);retryTimer=setTimeout(()=>switchProgram(cam),2500)}}
...
async function startInfrastructure(){
  ...
  try{
    status("V17.8 · CAM1 VISIBLE", "Conectando Firebase y TURN…");
    ...
    status("V17.8 · CAM1 MASTER LISTA",isInstalled()?"App instalada · inicia PRISM ScreenCast y vuelve aquí":"Instala la app para ocultar la barra de Chrome");showControls();
  }catch(e){
    infrastructureStarted=false;
    status("V17.8 · CÁMARA ACTIVA · INFRAESTRUCTURA PENDIENTE",e?.message||String(e));
