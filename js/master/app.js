...
async function promote(s,token){if(token!==switchToken||pendingRemote!==s)return;const old=activeRemote;activeRemote=s;pendingRemote=null;setRenderSource(s.video);program=s.cam;setProgramAudio(s.stream);status(`V16.1 · ${s.cam.toUpperCase()} AL AIRE`);if(replayEnabled)startBuffer(s.stream);setTimeout(()=>destroyRemote(old),250)}
...
async function switchProgram(cam){requestedCam=cam||"cam1";const token=++switchToken;if(pendingRemote){await destroyRemote(pendingRemote);pendingRemote=null}if(cam==="cam1"){const old=activeRemote;activeRemote=null;program="cam1";setRenderSource(local);setProgramAudio(null);status("V16.1 · CAM1 LOCAL AL AIRE");if(replayEnabled)startBuffer(localStream);setTimeout(()=>destroyRemote(old),250);return}if(activeRemote?.cam===cam)return;const target=activeRemote?.video===front?back:front;status(`V16.1 · preparando ${cam.toUpperCase()}`);try{pendingRemote=await buildRemote(cam,target,token)}catch(e){status(`No conecta ${cam}: ${e.message}`);clearTimeout(retryTimer);retryTimer=setTimeout(()=>switchProgram(cam),2500)}}
function forceOverlayTransparency(){
  try{
    ...
    const scoreScale=Math.max(0.24,Math.min(0.42,targetWidth/980));
    if(!style){
    ...
    style.textContent=`
      ...
      body.v10-embedded #overlay-root{top:18px!important;left:20px!important;right:auto!important;bottom:auto!important;width:980px!important;min-width:980px!important;animation:none!important;transition:none!important;filter:none!important;}
      body.v10-embedded #overlay-scale{transform:scale(${scoreScale})!important;transform-origin:top left!important;}
      body.v10-embedded #overlay-bar{box-shadow:0 6px 18px rgba(0,0,0,.55)!important;}
...
async function startInfrastructure(){
  ...
  try{
    status("V16.1 · CAM1 VISIBLE", "Conectando Firebase y TURN…");
    ...
    let overlayState={enabled:true},overlayOutputMode="master",overlayLayout={sizePct:28,position:"top-left",marginX:2,marginY:3},activeScene="game",interviewCfg={name:"",role:""};
    const applyOverlayDocumentLayout=()=>{try{
      ...
      const target=Math.min(500,Math.max(220,innerWidth*pct/100));
      const scale=target/980;
      const pos=overlayLayout.position||"top-left";
      let st=d.getElementById("v161-broadcast-layout");
      if(!st){st=d.createElement("style");st.id="v161-broadcast-layout";(d.head||d.documentElement).appendChild(st)}
      ...
      st.textContent=`
        html,body{margin:0!important;width:100vw!important;height:100vh!important;overflow:hidden!important;background:transparent!important;}
        body.cam1-master-layout #overlay-root{position:fixed!important;${xSide}${ySide}width:980px!important;min-width:980px!important;height:auto!important;transform:scale(${scale})!important;transform-origin:${pos.includes("right")?"top right":"top left"}!important;animation:none!important;transition:opacity .25s ease!important;filter:none!important;z-index:1000!important;}
        body.cam1-master-layout #overlay-scale{transform:none!important;transform-origin:top left!important;}
        body.cam1-master-layout #overlay-bar{box-shadow:0 6px 18px rgba(0,0,0,.55)!important;}
        body.cam1-master-layout #overlay-bar::before{display:none!important;animation:none!important;}
      `;
    ...
    db.ref(`switcher/${game}/broadcast/overlay`).on("value",snap=>{overlayState=snap.val()||{};let url=(overlayState.url||Switcher.app.defaultOverlayUrl||"").trim();if(!url||/beisbol-overlay\.web\.app\/overlay\.html/i.test(url))url=new URL("../overlay/index.html?embedded=1",location.href).href;if(overlay.src!==url)overlay.src=url;paintGraphics()});
    ...
    db.ref(`switcher/${game}/broadcast/replayCommand`).on("value",s=>{const c=s.val();if(!c?.id||c.id===lastCommand)return;lastCommand=c.id;c.action==="play"?playReplay(+c.start||0,+c.end||10,+c.speed||.5):c.action==="testTransition"?showTransition():finishReplay(false)});
    ...
    status("V16.1 · CAM1 MASTER LISTA",isInstalled()?"App instalada · inicia PRISM ScreenCast y vuelve aquí":"Instala la app para ocultar la barra de Chrome");showControls();
  }catch(e){
    infrastructureStarted=false;
    status("V16.1 · CÁMARA ACTIVA · INFRAESTRUCTURA PENDIENTE",e?.message||String(e));
